...
if ($_POST['note'.$_SESSION[recommendation10]] == null ) {
	...
	header('Location: noexp10.php');
			...
			$_SESSION['note2']=$_POST['note'.$_SESSION[recommendation10]];
			
               $stmt = $mysqli->prepare("INSERT INTO `Reponse` (`ref_participant`, `ref_movie`, `note1`, `note2`, `t_note1`, `t_note2`,`id_explanation`) VALUES (?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param("iiiissi", $_SESSION[id_parti],$_SESSION[recommendation10],$_SESSION[note1],$_SESSION[note2],$_SESSION[t1],$_SESSION[t2],$_SESSION[id_explanation10]);
		$stmt->execute();
		$stmt->close();
